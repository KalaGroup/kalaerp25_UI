...
import { PowderCoatingCheckerService } from './powdercoatingchecker.service';
import { Ipowdercoatingcheckerprcdts } from './Model/powdercoatingcheckerprcdts';
import { IpowdercoatingcheckerprcSave } from './Model/powdercoatingcheckerprcsave';
...
@Component({
  selector: 'app-powder-coating-checker',
  standalone: false,
  templateUrl: './powder-coating-checker.component.html',
  styleUrl: './powder-coating-checker.component.scss'
})
export class PowderCoatingCheckerComponent implements OnInit {

  // ---- Injected services ----
  // canopyService:   shared helpers (get6M, getToEmpNamePCCode).
  // pcCheckService:  powder-coating-checker–specific endpoints.
  private readonly canopyService = inject(CanopyProcessService);
  private readonly pcCheckService = inject(PowderCoatingCheckerService);
  ...
  // ---- Grid ----
  PcCheckerprcDetailsList: Ipowdercoatingcheckerprcdts[] = [];

  // ---- 6M popup ----
  showPopup = false;
  showProductionModal = false;
  selectedPC: Ipowdercoatingcheckerprcdts | null = null;
  selectedDG: any;
  remarks = '';
  ...
  private loadPlanCodes(): void {
    this.isLoading = true;
    this.pcCheckService.GetCheckerCPPlanLoad(this.PC).subscribe({
      next: (data: any[]) => {
  ...
  onClickSearch(): void {
    ...
    this.isLoading = true;
    this.pcCheckService
      .GetPowdercoating_chekerDetails(this.LoginCompCode, this.selectedPlanCode, this.PC)
      .subscribe({
        next: (data: Ipowdercoatingcheckerprcdts[]) => {
          this.PcCheckerprcDetailsList = data ?? [];
          this.isLoading = false;
        },
        error: (error) => {
          console.error(error);
  ...
  openCPCodePopup(powderCoating: Ipowdercoatingcheckerprcdts): void {
    this.selectedPC = powderCoating;
    this.selectedDG = powderCoating; // alias used by loadExistingData
  ...
  loadExistingData(): void {
    const dg = this.selectedDG ?? this.selectedPC;
    if (!dg) return;
  ...
  private submit(status: 'AUTH' | 'REJECT'): void {
    ...
    this.selectedPC.ProductionType = selectedItems.map((item) => item.name).join(', ');
    ...
    this.selectedPC.ProductionDetails = productionData.join('@@#@@');
    ...
    const payload: IpowdercoatingcheckerprcSave = {
      Code: '0',
      EmpCode: this.EmpCode ?? '',
      PCCode_Act: this.PC ?? '',
      PCCode: selectedLineObj?.ParentDgPC ?? '',
      CompCode: this.LoginCompCode ?? '',
      pfbCode: this.selectedPC.PFBCode ?? '',
      PlanCode: this.selectedPC.CanopyPlanCode ?? '',
      Sheetpartcode: this.selectedPC.Sheetpartcode ?? '',
      CatID: String(this.selectedPC.CatID ?? '0'),
      ProductCode: this.selectedPC.ProductCode ?? '',
      BatchQty: String(this.selectedPC.NestingForQty ?? 0),
      productionType: this.selectedPC.ProductionType ?? '',
      productionDetails: this.selectedPC.ProductionDetails ?? '',
      status,
    ...
    this.isLoading = true;
    this.pcCheckService.Postpowdercoating_chekerSave(payload).subscribe({
  ...
  private openResultPopup(action: 'AUTH' | 'REJECT', message: string, isError = false): void {
    ...
    this.resultTitle = isError
      ...
      : (action === 'REJECT' ? 'Job Card Rejected' : 'Powder Coating Process Authorized');
  ...
  /** Closes the popup and reloads the Powder Coating Checker page so dropdowns reset. */
  closeResultPopup(): void {
    this.showResultPopup = false;
    this.closePopup();
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/canopy-process/powder-coating-checker']);
    });
  }
}
