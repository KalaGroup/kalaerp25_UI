...
import { FabricationCheckerService } from './fabricationchecker.service';
import { Ifabricationcheckerprcdts } from './Model/fabricationcheckerprcdts';
import { IfabricationcheckerprcSave } from './Model/fabricationcheckerprcsave';
...
@Component({
  selector: 'app-fabrication-checker',
  standalone: false,
  templateUrl: './fabrication-checker.component.html',
  styleUrl: './fabrication-checker.component.scss'
})
export class FabricationCheckerComponent implements OnInit {

  // ---- Injected services ----
  // canopyService:    shared helpers (get6M, getToEmpNamePCCode).
  // fabCheckService:  fabrication-checker–specific endpoints.
  private readonly canopyService = inject(CanopyProcessService);
  private readonly fabCheckService = inject(FabricationCheckerService);
  ...
  // ---- Grid ----
  FabricationCheckerprcDetailsList: Ifabricationcheckerprcdts[] = [];

  // ---- 6M popup ----
  showPopup = false;
  showProductionModal = false;
  selectedFab: Ifabricationcheckerprcdts | null = null;
  selectedDG: any;
  remarks = '';
  ...
  loadLineByProcess(): void {
    this.canopyService.GetLineByProcess('Fabrication', this.LoginCompCode).subscribe({
  ...
  onLineSelect(lineWisePC: string): void {
    ...
    this.planCodes = [];
    this.FabricationCheckerprcDetailsList = [];
    this.loadPlanCodes();
  }
  ...
  private loadPlanCodes(): void {
    this.isLoading = true;
    this.fabCheckService.GetCheckerCPPlanLoad(this.PC).subscribe({
      next: (data: any[]) => {
  ...
  onClickSearch(): void {
    ...
    this.isLoading = true;
    this.fabCheckService
      .GetFabrication_chekerDetails(this.LoginCompCode, this.selectedPlanCode, this.PC)
      .subscribe({
        next: (data: Ifabricationcheckerprcdts[]) => {
          this.FabricationCheckerprcDetailsList = data ?? [];
          this.isLoading = false;
        },
        error: (error) => {
          console.error(error);
  ...
  openCPCodePopup(fabrication: Ifabricationcheckerprcdts): void {
    this.selectedFab = fabrication;
    this.selectedDG = fabrication; // alias used by loadExistingData
  ...
  loadExistingData(): void {
    const dg = this.selectedDG ?? this.selectedFab;
    if (!dg) return;
  ...
  private submit(status: 'AUTH' | 'REJECT'): void {
    ...
    this.selectedFab.ProductionType = selectedItems.map((item) => item.name).join(', ');
    ...
    this.selectedFab.ProductionDetails = productionData.join('@@#@@');

    // Line-wise PC (same as cnc-checker / bending-checker submit):
    ...
    const payload: IfabricationcheckerprcSave = {
      Code: '0',
      EmpCode: this.EmpCode ?? '',
      PCCode_Act: this.PC ?? '',
      PCCode: selectedLineObj?.ParentDgPC ?? '',
      CompCode: this.LoginCompCode ?? '',
      pfbCode: this.selectedFab.PFBCode ?? '',
      PlanCode: this.selectedFab.CanopyPlanCode ?? '',
      Sheetpartcode: this.selectedFab.Sheetpartcode ?? '',
      CatID: String(this.selectedFab.CatID ?? '0'),
      ProductCode: this.selectedFab.ProductCode ?? '',
      BatchQty: String(this.selectedFab.NestingForQty ?? 0),
      productionType: this.selectedFab.ProductionType ?? '',
      productionDetails: this.selectedFab.ProductionDetails ?? '',
      status,
    ...
    this.isLoading = true;
    this.fabCheckService.postFabrication_chekerSave(payload).subscribe({
  ...
  private openResultPopup(action: 'AUTH' | 'REJECT', message: string, isError = false): void {
    ...
    this.resultTitle = isError
      ...
      : (action === 'REJECT' ? 'Job Card Rejected' : 'Fabrication  Process Authorized');
  ...
  /** Closes the popup and reloads the Fabrication Checker page so dropdowns reset. */
  closeResultPopup(): void {
    this.showResultPopup = false;
    this.closePopup();
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/canopy-process/fabrication-checker']);
    });
  }
}
