...
import { ControlPanelJobcardCheckerService } from './control-panel-jobcard-checker.service';
import { IJobcardCpyChekerDts } from '../../canopy-process/sheet-metal-jobcard-checker/Model/jobCard_Cpy_chekerDts';
import { IToEmpNamePCCode } from '../../canopy-process/sheet-metal-jobcard-checker/Model/ToEmpNamePCCode';
...
@Component({
  selector: 'app-control-panel-jobcard-checker',
  standalone: false,
  templateUrl: './control-panel-jobcard-checker.component.html',
  styleUrl: './control-panel-jobcard-checker.component.scss'
})
export class ControlPanelJobcardCheckerComponent implements OnInit {
  ...
  constructor(
    ...
    private cpCheckerService: ControlPanelJobcardCheckerService,
  ...
  private loadPlanCodes(): void {
    this.isLoading = true;
    this.cpCheckerService.GetCheckerCPPlanLoad().subscribe({
      next: (data: any[]) => {
  ...
  onAccept(): void {
    ...
    this.isLoading = true;
    this.cpCheckerService.postControlPanelJobcardCheckerSave(payload).subscribe({
  ...
  closeResultPopup(): void {
    ...
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/control-panel/control-panel-jobcard-checker']);
    });
  }
  ...
  private buildBarcodeSvgMarkup(text: string, height = 50): string | null {
    ...
    const patterns = ControlPanelJobcardCheckerComponent.CODE39_PATTERNS;
