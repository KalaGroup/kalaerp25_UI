...
import { CncCpService } from './cnccp.service';
...
@Component({
  selector: 'app-cnc-cp-maker',
  standalone: false,
  templateUrl: './cnc-cp-maker.component.html',
  styleUrl: './cnc-cp-maker.component.scss'
})
export class CncCpMakerComponent implements OnInit {

  // ---- Injected services (Angular 19 inject API) ----
  private readonly cncService = inject(CncCpService);
  ...
  closeResultPopup(): void {
    ...
    this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
      this.router.navigate(['/control-panel/cnc-cp-maker']);
    });
  }
