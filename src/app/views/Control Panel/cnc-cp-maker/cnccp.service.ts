...
/**
 * CNC CP Maker service — dropdowns, plan, sheet & kit lookups, submit.
 * Dedicated to the Control Panel CNC maker (cnc-cp-maker) with its own models.
 */
@Injectable({ providedIn: 'root' })
export class CncCpService {
  ...
  private logUrl(method: string, label: string, url: string, params?: HttpParams | object): void {
    if (params instanceof HttpParams) {
      ...
      console.log(`[CncCpService] ${method} ${label} → ${url}${qs ? '?' + qs : ''}`);
    } else if (params && typeof params === 'object') {
      // eslint-disable-next-line no-console
      console.log(`[CncCpService] ${method} ${label} → ${url}`, params);
    } else {
      // eslint-disable-next-line no-console
      console.log(`[CncCpService] ${method} ${label} → ${url}`);
    }
  }
