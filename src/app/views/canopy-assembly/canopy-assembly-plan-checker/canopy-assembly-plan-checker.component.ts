...
export class CanopyAssemblyPlanCheckerComponent implements OnInit {
  ...
  //    Canopy Plan / Process / Process Checker / Flatpack forms). No
  ...
  private doSave(decision: 'Accept' | 'Rework' | 'Reject'): void {
    if (!this.ctxHeader) return;
    // Resolve the selected line ONCE so LineWisePC + ParentDgPC always
    // come from the same hardcoded row (no drift where PCCode is set
    // but ParentDgPC is accidentally blank).
